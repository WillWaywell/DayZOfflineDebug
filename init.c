...
{
  ref array<ref KeyBind> keyBinds = new array<ref KeyBind>;
  ...
  void CustomMission()
  {
    RegisterKeyBind(KeyCode.KC_HOME, -1, this, "TeleportHome");
    RegisterKeyBind(KeyCode.KC_INSERT, -1, this, "ToggleDebugCamera");
    RegisterKeyBind(KeyCode.KC_DELETE, -1, this, "ToggleSceneEditor");
    RegisterKeyBind(KeyCode.KC_T, -1, this, "TeleportToCursor");
    RegisterKeyBind(KeyCode.KC_O, KeyCode.KC_LSHIFT, this, "CreateAnimal");
    RegisterKeyBind(KeyCode.KC_O, -1, this, "CreateInfected");
    RegisterKeyBind(KeyCode.KC_END, -1, this, "ToggleGodMode");
    GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).CallLater(OnTick, 0, true);
  }
  ...
  void RegisterKeyBind(int key, int modifierKey, Class inst, string function)
  {
    keyBinds.Insert(new KeyBind(key, modifierKey, inst, function));
  }
  ...
  void TeleportHome()
  {
    ...
    GetGame().Chat("Teleported home", "colorAction");
  }
  ...
  void CreateInfected()
  {
    GetGame().CreateObject(ConfigGetRandomChildFromBase("cfgVehicles", "DayZInfected"), player.GetPosition() + Vector(Math.RandomFloatInclusive(-5, 5), 0, Math.RandomFloatInclusive(-5, 5)), false, true);
    GetGame().Chat("Created random infected", "colorAction");
  }

  void CreateAnimal()
  {
    GetGame().CreateObject(ConfigGetRandomChildFromBase("cfgVehicles", "AnimalBase"), player.GetPosition() + Vector(Math.RandomFloatInclusive(-5, 5), 0, Math.RandomFloatInclusive(-5, 5)), false, true);
    GetGame().Chat("Created random animal", "colorAction");
  }
  ...
  void ToggleGodMode()
  {
    if (!godModeEnabled)
    {
      ...
      GetGame().Chat("God mode ENABLED", "colorAction");
      godModeEnabled = true;
    }
    else
    {
      player.SetAllowDamage(true);
      GetGame().Chat("God mode DISABLED", "colorAction");
      godModeEnabled = false;
    }
  }

  string ConfigGetRandomChildFromBase(string configName, string baseName)
  {
    int count = GetGame().ConfigGetChildrenCount(configName);
    string childName = "";
    ...
    {
      GetGame().ConfigGetChildName(configName, i, childName);

      if (GetGame().IsKindOf(childName, baseName))
      {
        classNames.Insert(childName);
      }
    }
...
class KeyBind
{
  ...
  void KeyBind(int key, int modifierKey, Class inst, string function)
  {
    this.key = key;
  ...
  bool IsPressed()
  {
    return KeyState(key) == 1 && (modifierKey == -1 || KeyState(modifierKey) == 1)
  }

  void Callback()
  {
    GetGame().GameScript.CallFunction(inst, function, NULL, 0);
  }

  void ClearKeyPress()
  {
    ClearKey(key);
    //if (modifierKey != -1) ClearKey(modifierKey);
  }
}
